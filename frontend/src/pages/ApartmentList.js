...
const ApartmentList = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        const res = await axios.get("http://localhost:5000/api/apartments", { headers:{ Authorization:`Bearer ${user?.token}`}});
        setList(res.data);
    ...
    // SSE for apartments
    const es = new EventSource("http://localhost:5000/sse/apartments");
